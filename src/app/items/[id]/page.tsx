...
import { getItemById } from "../_action";
...
export default function CreateItem() {
  ...
  useEffect(() => {
    ...
    // Assuming the item ID is part of the pathname, e.g., "/items/[id]"
    const parts = pathname.split("/"); // Split the pathname
   
    const idIndex = parts.indexOf("items") + 1; // Get index of item ID
    console.log()
    const fetchItem = async () => {
      const item = await getItemById(`${idIndex}`);
      console.log("item details", item, `${idIndex}`);
    }
    
    fetchItem();
    // if (idIndex > 0 && idIndex < parts.length) {
    //   setItemId(parts[idIndex]); // Set the item ID from the pathname
    // }
  }, [pathname]);
  ...
  useEffect(() => {
    // Fetch the item only if the ID is available

    console.log(itemId)
    // if (id) {
    //   const fetchItem = async () => {
    //     const item = await getItemById(id as string); // Ensure id is of type string
    //     if (item) {
